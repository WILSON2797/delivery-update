...
try {
    ...
    // 9. Total Amount (sum dari semua total_amount)
    $queryTotalAmount = "SELECT SUM(total_amount) as total FROM billing_details WHERE total_amount IS NOT NULL";
    $resultTotalAmount = $conn->query($queryTotalAmount);
    $totalAmount = $resultTotalAmount->fetch_assoc()['total'] ?? 0;
