function initPageScripts() {
    ...
    function initEditDropdowns(motValue = null, statusValue = null, driverValue = null) {
        console.log(`🔄 Loading edit dropdowns: MOT=${motValue}, Status=${statusValue}, Driver=${driverValue}`);
        ...
        const promises = [
            ...
            loadDriverDropdown($('#edit_driver_name'), '#editReportModal', driverValue)
        ];
        
        return Promise.all(promises);
    }
    
    // Auto-fill Nopol & Phone saat driver dipilih
    $(document).on('change', '#edit_driver_name', function () {
    ...
    // btp (back to pool code)
    $(document).on('change', '#edit_btp_datetime_date, #edit_btp_datetime_time', function() {
        combineDateTime('edit_btp_datetime_date', 'edit_btp_datetime_time', 'edit_btp_datetime');
    });
            ...
            columns: [
                ...
                },
                {
                    ...
                    render: function (data) {
                        ...
                        else if (data === "Planned") color = "info";
    ...
    const btpDatetime = $('#edit_btp_datetime').val();
    ...
    // Validasi 5: Jika status "Back To Pool", Back To Pool DateTime wajib diisi
    if (status === "Back To Pool" && !btpDatetime) {
        showErrorToast("Validasi Gagal", "Back To Pool DateTime wajib diisi ketika status Back To Pool!");
        return false;
    }
    ...
    function generateTimeline() {
        const timelineFields = [
            { id: 'edit_truck_on_warehouse', label: 'Truck on Pickup Point' },
            { id: 'edit_atd_whs_dispatch', label: 'Done Pickup' },
            { id: 'edit_atd_pool_dispatch', label: 'OTW To Site' },
            { id: 'edit_ata_mover_on_site', label: 'Mover Onsite' },
            { id: 'edit_pod_datetime', label: 'HO Done' },
            { id: 'edit_btp_datetime', label: 'Back To Pool' }
        ];
    ...
    $(document).off("click.editReport", ".edit-report")
           .on("click.editReport", ".edit-report", function () {
        ...
        $.ajax({
            ...
            success: function (res) {
                if (res.status === "success") {
                    ...
                    splitDateTime(d.btp_datetime, 'edit_btp_datetime_date', 'edit_btp_datetime_time');
                    ...
                    $("#edit_btp_datetime").val(d.btp_datetime || "");
                    ...
                    const timelinePattern = /^\d{2}\/\d{2}\/\d{4}\s+\d{2}:\d{2}(:\d{2})?\s*:\s*(Truck on Pickup Point|Done Pickup|OTW To Site|Mover Onsite|HO Done | Back To Pool)$/;
                    ...
                    initEditDropdowns(d.mot || "", d.status || "", d.driver_name || "")
                        .then((results) => {
    ...
    $(document).on('change', 
        ...
        '#edit_btp_datetime_date, #edit_btp_datetime_time, ' +
    ...
    $(document).on('change', '#edit_status', function() {
        ...
        const btpWrapper = $('#btp_datetime_wrapper');
        const btpDateField = $('#edit_btp_datetime_date');
        const btpTimeField = $('#edit_btp_datetime_time');
        const btpHiddenField = $('#edit_btp_datetime');
        
        if (status === "Back To Pool") {
            // Tampilkan field dan set required
            btpWrapper.slideDown(300);
            btpDateField.attr('required', true);
            btpTimeField.attr('required', true);
        } else {
            // Sembunyikan field dan hapus required + reset value
            btpWrapper.slideUp(300);
            btpDateField.removeAttr('required').val('');
            btpTimeField.removeAttr('required').val('');
            btpHiddenField.val('');
        }
    
        // Existing POD datetime logic
