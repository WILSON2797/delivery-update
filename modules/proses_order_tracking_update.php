...
try {
    ...
    // UBAH: Sekarang wajib transaction_id
    ...
    try {
        ...
        // UBAH: Lock berdasarkan transaction_id
        $lockCheck = $conn->prepare("SELECT id, dn_number FROM daily_report WHERE transaction_id = ? FOR UPDATE");
        ...
        // UBAH: Update berdasarkan transaction_id
        ...
        // Parameter untuk daily_report (total 44 field + 1 WHERE transaction_id = 45)
        $params_daily = [
            $date_request,                          // 1
            ...
            $overnight_day,                         // 43
            $update_by,                             // 44
            $transaction_id                         // 45 (WHERE) - UBAH: Pakai transaction_id
        ];
        ...
        // UBAH: Cek berdasarkan transaction_id
        ...
        // Konversi pod_datetime ke pod_date dan year
        $pod_date = null;
        $year = null;
        ...
        if ($billing_exists) {
            ...
            $stmt_billing->bind_param(
                ...
                $transaction_id                     // 14 - WHERE transaction_id - UBAH
            );
            ...
            $stmt_billing_insert->bind_param(
                "sssssissssssss",
                $transaction_id,                    // 1 - transaction_id - TAMBAH
                $new_dn_number,                     // 2 - dn_number
                $billing_sub_project,               // 3 - sub_project
                $pod_date,                          // 4 - pod_date
                $status,                            // 5 - status
                $year,                              // 6 - year
                $gross_weight,                      // 7 - weight
                $gross_weight,                      // 8 - weight_chargeable
                $billing_type_shipment,             // 9 - type_shipment
                $mot,                               // 10 - mot
                $plan_from,                         // 11 - plan_from
                $billing_destination_province,      // 12 - destination_province
                $billing_destination_city,          // 13 - destination_city
                $billing_destination_address        // 14 - destination_address
            );
