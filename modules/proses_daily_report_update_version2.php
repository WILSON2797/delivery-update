...
try {
    ...
    try {
        ...
        $lockCheck = $conn->prepare("SELECT dn_number, type_shipment, gross_weight FROM daily_report WHERE id = ? FOR UPDATE");
        ...
        $existing_type_shipment = $row['type_shipment']; // Simpan type_shipment yang ada
        $existing_gross_weight = $row['gross_weight']; // Simpan gross_weight yang ada
        $lockCheck->close();
        ...
        // Cek apakah record sudah ada di billing_details
        $check_billing = $conn->prepare("SELECT id FROM billing_details WHERE dn_number = ?");
        $check_billing->bind_param("s", $dn_number);
        ...
        if ($billing_exists) {
            ...
            // HANYA UPDATE FIELD YANG ADA DI FORM UPDATE
            ...
            // INSERT billing_details jika belum ada (untuk data lama)
            // Ambil data lengkap dari daily_report untuk insert
            ...
            if ($data) {
                ...
                $stmt_billing_insert->bind_param(
                    ...
                    $existing_gross_weight,          // Gunakan data existing
                    $existing_gross_weight,          // Gunakan data existing
                    $existing_type_shipment,         // Gunakan data existing
                    $mot_billing,
                    $pod_date,
                    $status
                );
        ...
        echo json_encode([
            'status' => 'success',
            'message' => 'Data delivery report dan billing details berhasil diupdate!',
            'dn_number' => $dn_number
        ]);
